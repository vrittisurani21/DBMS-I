...
SELECT YEAR, PRODUCT, SUM(SALES_AMOUNT) AS SUM_AMOUNT FROM SALES_DATA GROUP BY YEAR, PRODUCT

--7.Display Regions with Total Sales Greater Than 5000 
SELECT REGION, SUM(SALES_AMOUNT) AS SUM_AMOUNT FROM SALES_DATA GROUP BY REGION HAVING SUM(SALES_AMOUNT)>5000

--8.Display Products with Average Sales Less Than 10000
SELECT PRODUCT, AVG(SALES_AMOUNT) AS AVG_AMOUNT FROM SALES_DATA GROUP BY PRODUCT HAVING AVG(SALES_AMOUNT)<10000

--9.Display Years with Maximum Sales Exceeding 500 
SELECT YEAR, MAX(SALES_AMOUNT) AS MAX_AMOUNT FROM SALES_DATA GROUP BY YEAR HAVING MAX(SALES_AMOUNT)>500

--10.Display Regions with at Least 3 Distinct Products Sold.
SELECT REGION,  COUNT(PRODUCT) FROM SALES_DATA GROUP BY REGION HAVING COUNT(DISTINCT PRODUCT)>=3

--11.Display Years with Minimum Sales Less Than 1000 
SELECT YEAR, MIN(SALES_AMOUNT) AS MIN_AMOUNT FROM SALES_DATA GROUP BY YEAR HAVING MIN(SALES_AMOUNT)<1000
...
SELECT YEAR, REGION, COUNT(PRODUCT) AS COUNT_PRODUCT FROM SALES_DATA GROUP BY REGION, YEAR ORDER BY REGION, YEAR
...
SELECT YEAR, SUM(SALES_AMOUNT) AS TOTAL_AMOUNT FROM SALES_DATA GROUP BY YEAR HAVING SUM(SALES_AMOUNT)<1000 ORDER BY YEAR DESC 

--4. Display Top 3 Regions by Total Sales Amount in Year 2024
SELECT REGION, SUM(SALES_AMOUNT) AS TOTAL FROM SALES_DATA WHERE YEAR=2024 GROUP BY REGION 
...
SELECT PRODUCT, AVG(SALES_AMOUNT) AS AVG_AMOUNT FROM SALES_DATA WHERE SALES_AMOUNT BETWEEN 1000 AND 2000 GROUP BY PRODUCT ORDER BY PRODUCT

--2. Display Years with More Than 5 Orders from Each Region


--3. Display Regions with Average Sales Amount Above 1500 in Year 2023 sort by amount in descending.
SELECT REGION, AVG(SALES_AMOUNT) AS AVG_AMOUNT FROM SALES_DATA WHERE SALES_AMOUNT > 1500 YEAR=2023 GROUP BY REGION ORDER BY SALES_AMOUNT DESC
